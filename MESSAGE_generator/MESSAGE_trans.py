...
interconnection_long = pd.melt(interconnection, id_vars = interconnection.columns[0], value_vars = interconnection.columns[1:])
...
interconnection_main = interconnection_long[interconnection_long['from'].isin(province_list) & interconnection_long['to'].isin(province_list) ]
#FuelPrice
...
for cid, c in enumerate(cases_all):
    ...
    if c in province_list:
        ...
        if generate_main == 1:
            province_dict_1[province[0]] = province_dict_1.get(province[0], 0)+1 #count number of times the first letter appears
            province_dict[province] = province[0]+str(province_dict_1[province[0]]) #create region id with first letter of province and number of times it occurs
...
if generate_main == 1:
    ...
    #DONE create a main folder
    #create interconnector technologies
    #DONE using last region, create the files
    #DONE create regid file
    #create main bat file
    #DONE also add to reg

    #cin file, hourly
    #convert from cin file to excel
...
# +f"    units	group: activity, type: weight, cost:US$'00/ton, upper:kton, lower:kton"
# +f"    for_ldr	none"
# +f"    upper	ts 43.55 43.55 43.55 43.55 43.55 43.55 43.55 43.55 43.55 43.55 0 0"
# +f"    lower	c 0"
# +f"    type	None"
# +*

#adb in region
#con1a CO2L	c 9.439693647
